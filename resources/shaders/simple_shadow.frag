...
{
#ifdef TAA_PASS_DYNAMIC
  out_velocity = (surf.clipPosCur.xy / surf.clipPosCur.w - surf.clipPosPrev.xy / surf.clipPosPrev.w) * 0.5;
#endif
  ...
  const float shadow    = ((posLightSpaceNDC.z < textureLod(shadowMap, shadowTexCoord, 0).x + 0.001f) || outOfView) ? 1.0f : 0.1f;
