...
void main()
{
  ...
  vec3 towardFragDir = normalize(surf.wPos - Params.lightPos);
  float cos_angle = dot(towardFragDir, normalize(Params.lightDir));

  float diff = Params.cos_inner_angle - Params.cos_outer_angle;
  float intensity = clamp((cos_angle - Params.cos_outer_angle) / diff, 0.0, 1.0);  
