...
void DeferredRender::InitPresentStuff()
{
  ...
  m_pTextureLoader = std::make_shared<etna::TextureLoader>(vk_utils::createCommandBuffer(m_context->getDevice(), m_commandPool));
...
void DeferredRender::ResetPresentStuff()
{
  ...
  if (*m_pTextureLoader->getCommandBufferPtr() != VK_NULL_HANDLE) {
    vkFreeCommandBuffers(m_context->getDevice(), m_commandPool, 1, m_pTextureLoader->getCommandBufferPtr());
  }
...
void DeferredRender::InitPresentation(VkSurfaceKHR &a_surface, bool)
{
  m_surface = a_surface;
